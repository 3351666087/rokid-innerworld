...
async function assertUnityAdapterBoundary() {
  ...
  assert(/private string BuildRuntimeContractLine\(\)[\s\S]*BuildWallCalibrationStatusLine[\s\S]*BuildFieldMarkerStatusLine/.test(controller), "Unity controller HUD runtime line must expose wall calibration and field marker status");
  assert(/private string BuildWallCalibrationHeartbeatLine\(\)[\s\S]*BuildWallCalibrationObservationLine[\s\S]*BuildFieldMarkerHeartbeatLine/.test(controller), "Unity controller heartbeat payload must expose last calibration observation and field marker status");
  assert(/private void RefreshTargetHud\(\)[\s\S]*BuildWallCalibrationObservationLine[\s\S]*BuildFieldMarkerActiveLine/.test(controller), "Unity controller target HUD must expose last calibration observation and active field marker");
  assert(/private void RefreshInputStatusLine\(\)[\s\S]*BuildWallCalibrationObservationLine[\s\S]*BuildFieldMarkerReadinessLine/.test(controller), "Unity controller device/input status must expose calibration observation and field marker readiness");
  assert(/BuildSdkBindingStatusPayload[\s\S]*message = BuildWallCalibrationHeartbeatMessage\(report.Message\)/.test(controller), "Unity controller heartbeat payload must include wall calibration status");
