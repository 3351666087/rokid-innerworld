...
async function assertUnityAdapterBoundary() {
  ...
  assert(/private string BuildRuntimeContractLine\(\)[\s\S]*BuildWallCalibrationStatusLine/.test(controller), "Unity controller HUD runtime line must expose wall calibration status");
  assert(/private string BuildWallCalibrationHeartbeatLine\(\)[\s\S]*BuildWallCalibrationObservationLine/.test(controller), "Unity controller heartbeat payload must expose last calibration observation");
  assert(/private void RefreshTargetHud\(\)[\s\S]*BuildWallCalibrationObservationLine/.test(controller), "Unity controller target HUD must expose last calibration observation");
  assert(/private void RefreshInputStatusLine\(\)[\s\S]*BuildWallCalibrationObservationLine/.test(controller), "Unity controller device/input status must expose last calibration observation");
