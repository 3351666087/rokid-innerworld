...
async function main() {
  ...
  assert(evidenceChain.evidence_replay_judge_mode?.schema === "innerworld-evidence-replay-judge-mode/v1", "evidence replay judge mode schema check failed");
  assert(evidenceChain.evidence_replay_judge_mode?.mode === "evidence_replay_judge_mode", "evidence replay judge mode name check failed");
  assert(evidenceChain.evidence_replay_judge_mode?.read_only === true, "evidence replay judge mode read-only check failed");
  assert(evidenceChain.evidence_replay_judge_mode?.sequence?.join(" -> ") === "A1 entry -> A2 memory -> A3 writeback -> User B readback", "evidence replay judge sequence failed");
  assert(Array.isArray(evidenceChain.evidence_replay_judge_mode?.replay_steps) && evidenceChain.evidence_replay_judge_mode.replay_steps.length === 4, "evidence replay judge steps failed");
  assert(evidenceChain.evidence_replay_judge_mode.replay_steps.map((step) => step.id).join(",") === "a1_entry,a2_memory,a3_writeback,user_b_readback", "evidence replay judge step ids failed");
  assert(evidenceChain.evidence_replay_judge_mode.source_evidence?.some((source) => source.id === "sqlite_runtime"), "evidence replay SQLite source failed");
  assert(evidenceChain.evidence_replay_judge_mode.source_evidence?.some((source) => source.id === "writeback_api"), "evidence replay writeback source failed");
  assert(evidenceChain.evidence_replay_judge_mode.source_evidence?.some((source) => source.id === "field_acceptance"), "evidence replay field acceptance source failed");
  assert(evidenceChain.evidence_replay_judge_mode.source_evidence?.some((source) => source.id === "release_chain"), "evidence replay release source failed");
  assert(evidenceChain.evidence_replay_judge_mode.privacy?.includes_secrets === false, "evidence replay secrets privacy flag failed");
  assert(evidenceChain.evidence_replay_judge_mode.privacy?.includes_raw_chat === false, "evidence replay raw chat privacy flag failed");
  assert(evidenceChain.evidence_replay_judge_mode.privacy?.includes_runtime_db_dump === false, "evidence replay runtime DB privacy flag failed");
  assert(evidenceChain.evidence_replay_judge_mode.privacy?.includes_private_ids === false, "evidence replay private IDs privacy flag failed");
  assert(evidenceChain.evidence_items.some((item) => item.id === "evidence_replay_judge_mode"), "evidence chain judge mode item check failed");
