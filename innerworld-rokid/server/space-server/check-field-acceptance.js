...
const requiredGateIds = ["print_kit", "simulator_rehearsal", "hardware_alignment"];
...
function assertAcceptancePayload(payload) {
  ...
  assert(payload.summary?.ready_for_hardware === false || payload.summary?.hardware_evidence_count > 0, "hardware ready requires hardware evidence");
  assert(payload.summary?.simulator_rehearsal_is_not_hardware_ready === true, "simulator rehearsal hardware separation missing");
  assert(hardware.evidence?.ready_for_hardware === payload.summary.ready_for_hardware, "hardware summary and gate evidence mismatch");
...
async function assertCheckIsExposed() {
  ...
  const [mainline, contract, webDemo] = await Promise.all([
