...
Mission/write-back acceptance now also requires trusted mission provenance. Unity/Rokid interaction, service action, TimeMark write-back, and User B readback requests carry session/device/anchor inputs for server-side proof, while public ledger/API output keeps raw session ids, raw device ids, private network identifiers, pairing codes, raw pose streams, and raw ray vectors out. `mission_loop_ready=true` only means the P0 mission ledger/provenance side is satisfied; it still cannot make physical acceptance true while trusted A1/A2/A3 observations are missing.
