...
- Current authoritative APK fact: `output/unity-android/InnerWorldRokid.apk`, 45,727,279 bytes, SHA256 `19733d32b2bdbd347895a319e55a051c7c3722d1329f823baf78ce61e9978955`. It includes `assets/RKImage.db`, `libopenxr_loader.so`, `librokid_openxr_api.so`, and `libyuv.so`, and the APK metadata contains the `IW_TARGET_*` target-observation diagnostics plus the latest Unity mission provenance payload code for the next physical wall pass.
...
- Current mutating Station Pro proof for that exact `19733d32b2bd...` SHA is green: install OK, cold launch OK, process observed, `is_uxr_app=true`, operator pairing issued/injected/verified, and the launch evidence includes no raw pairing code, raw session id, raw serial, raw USB id, private IP, or MAC address.
...
- Current `field:target-pass:apply` now attaches current online operator-paired live session/device/anchor provenance inputs when it posts A2 read/find_year, controlled service action, A3 TimeMark, or User B readback. Reports expose only sanitized `provenance_input` summaries with hash prefixes and input-confirm status, not raw session/device ids.
...
- Historical note: the 22:18 `ce3f...`, 21:05/22:01 `70592...`, 2026-07-04 12:29 `bd852...`, 2026-07-04 14:31/15:25 `e447...`, and 2026-07-04 16:17 `9ddf...` APK facts are superseded by the current `19733d32b2bd...` APK and must not be used as the current next proof. The old `fallback_only`/unpaired boundary describes earlier evidence, not the current pair-smoke/live-session state.
...
- Earlier direct Gradle rebuild completed after adding Aliyun Maven mirrors to Unity's generated Gradle `settings.gradle`: `:launcher:assembleRelease` succeeded and `launcher-release.apk` was copied to `output/unity-android/InnerWorldRokid.apk` (24,752,749 bytes). This is historical LAN package evidence; the current disk APK is now the 45,727,279-byte `19733d32b2bd...` artifact recorded above.
