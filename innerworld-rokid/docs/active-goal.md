...
Current continuation: `field:acceptance-session` now embeds that operator-plan snapshot into the閻滄澘婧€ session report. The wrapper still does not create simulator/manual observations; operator-plan is a read-only phase/blocker guide, and hardware-ready remains gated by trusted A1/A2/A3, A3 write-back, User B readback, and `/api/field/acceptance`.
...
- Current live/target Markdown reports now expose the mission split at the top: `mission_ledger_ready`, trusted A1/A2/A3 prerequisite readiness, missing trusted anchors, and trusted mission provenance. The runbook fail-fast list includes `mission_loop_waiting_for_trusted_a1_a2_a3`, so閻滄澘婧€ operators do not treat a completed ledger as physical acceptance.
...
Fresh re-audit requested on 2026-07-03 19:40 Asia/Shanghai completed in `.agents/carver/`. The final record is `.agents/carver/source-audit-2026-07-03.md`; Carver rendered `docs/缁涙牕鍨濆?pdf` into 13 page PNGs, smoke-rendered 19 raw PDFs, and confirmed the root proposal PDF is readable with `pypdf`/`pdfplumber`.
...
- `docs/缁涙牕鍨濆?pdf` is represented by the final #249 proposal extraction for content-level work. If visual fidelity becomes a claim, render all 13 pages to PNG and inspect/OCR before citing layout details.
...
Rokid InnerWorld / 闂€婊嗩潌閻ㄥ嫰鏆遍張鐔烘窗閺嶅洦妲搁敍姝妎kid 閻ジ鏆呴崷銊ф埂鐎圭偞鐗庨崶顓炵潔婢ф瑤绗傞幍鎾崇磻娑撯偓鐏炲倸褰茬拠姹団偓浣稿讲閸愭瑣鈧礁褰叉径宥堫問閻ㄥ嫮鈹栭梻纾嬵唶韫囧棗鐪伴妴渚? 閸欘亝婀囬崝鈥茬闂堛垹褰查幒褎鐗庨崶顓炵潔婢ф瑤绗傞惃?A1/A2/A3 娑撳鏁嬮悙褰掓４閻滎垽绱癆1 閸忋儱褰涢幍鎾崇磻缁屾椽妫跨仦鍌︾礉A2 鐠囪褰囬弽鈥虫疮鐠佹澘绻?娴犺濮熺痪璺ㄥ偍閿涘瓑3 閸愭瑥娲?TimeMark閿涘苯鑻熺拋鈺佹倵閺夈儴鈧?User B 閸︺劌鎮撴稉鈧粚娲？閻鍩岄弬鏉款杻鐠佹澘绻傞妴?
...
## Current checkpoint - shiyao-first spatial choreography

- Real hardware remains with teammate shiyao; this branch does not claim local live hardware-ready.
- The post-scan concrete scene now has data-driven spatial choreography for A1/A2/A3/User B: depth=time lanes, wall-seeded growth beats, gesture affordances, and spatial sound cues.
- Unity and web fallback consume the same scene_actions[*].spatial_choreography contract; gates must pass before packaging or merge.
## Current checkpoint - executable scene targets

- A1/A2/A3/User B scene actions now include `task_target` contracts with endpoint sequences and explicit `fallback_no_hardware_claim` guards.
- Unity scene action nodes can be clicked or gaze-selected to execute A1 local entry confirm, A2 read/find_year, A3 service action + TimeMark write-back, and User B readback through existing Space API paths.
- Web fallback cards execute the same flow for localhost/LAN rehearsal while preserving the rule that fallback/manual/simulator does not prove hardware readiness.
