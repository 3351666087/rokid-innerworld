...
- Current live-pass snapshot after the 2026-07-04 17:51 current-APK pair smoke proves the operator-paired live SDK session window is open (`live_session_ready=true`, one online live operator-paired session). Current ledger/provenance can report `mission_loop_ready=true`, `user_b_readback_ready=true`, and `trusted_mission_provenance_ready=true`, but it still correctly reports `trusted_a1_a2_a3_ready=false`, `hardware_a1_a2_a3_ready=false`, and `field_acceptance_ready=false`.
